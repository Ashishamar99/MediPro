...
const getAppointmentList = (_req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        return res.json({
            status: status_1.Status.SUCCESS, data: yield prisma_1.default.appointment.findMany({
                include: {
                    slot: true
                }
            })
...
const createAppointment = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        if (!slot || slot.status === 'booked') {
            return res.status(404).json({ status: status_1.Status.FAILED, message: 'Slot not found' });
        }
        ...
        if (!patient) {
            return res.status(404).json({ status: status_1.Status.FAILED, message: 'Patient not found' });
        }
        ...
        return res.status(201).json({ status: status_1.Status.SUCCESS, message: 'Appointment created successfully', data });
    }
    catch (err) {
        console.log(err);
...
const cancelAppointment = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        if (!existingAppointment) {
            return res.status(404).json({ status: status_1.Status.FAILED, message: 'Appointment not found' });
        }
        ...
        return res.status(200).json({ status: status_1.Status.SUCCESS, message: 'Appointment cancelled successfully', data: result });
    }
    catch (err) {
        console.error(err);
        return res.status(500).json({ status: status_1.Status.ERROR, message: err });
